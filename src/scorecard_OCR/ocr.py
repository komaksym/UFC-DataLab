...
class FightData:
    ...
    def to_list(self) -> List:
        """Converts the data class to a list format."""
    ...
    def validate(self) -> bool:
        """Validates fight data
        Fails if the points is not of length 3, a name doesn't exist
        or the data is neither a complete number nor just empty placeholder"""

        try:
            if (
                len(self.red_fighter_total_pts) != 3
                or len(self.blue_fighter_total_pts) != 3
                or self.red_fighter_name == "-"
                or self.blue_fighter_name == "-"
                or not all(
                    p.isdigit() or p == "-" for p in self.red_fighter_total_pts + self.blue_fighter_total_pts
                )
            ):
                return False
            return True
...
def save_results(collected_results: List[FightData], save_path: Path) -> pd.DataFrame:
...
def process_scorecards(
    input_path: Path, output_path: Path, num_workers: int = DEFAULT_NUM_WORKERS
) -> pd.DataFrame:
    ...
    try:
        ...
        with Pool(num_workers) as pool:
            for result in tqdm(pool.imap(parse_image, images), total=len(images), desc="Processing images"):
