...
class ScorecardParser():
    """Handles the processing of scorecard images. """
    def __init__(self, lang: str = 'en', use_angle_cls: bool = False):
        self.ocr = PaddleOCR(use_angle_cls, lang)

    def parse_image(self, image_path: str) -> FightData:
        """Parsing the image."""
        try:
            ...
            logging.error(f"Errorr processing {image_path}: {str(e)}")
            return fight_data
...
def test_points_values(red_fighter_total_pts, blue_fighter_total_pts):
    """Unit testing red/blue_fighter_total_pts."""
...
def run_unit_tests(red_fighter_total_pts, blue_fighter_total_pts,
    ...
    """Main wrapper for all unit tests"""
...
def process_scorecards(folder_path: Path, output_path: Path, 
                       batch_size: int = 100, num_workers: int = 8
                       ) -> pd.DataFrame:
    ...
    try:
        ...
        scorecards_parser = ScorecardParser()
        collected_results = []
        
        with Pool(num_workers) as pool:
            with tqdm(total=len(images), desc="Total progress") as pbar:
                for batch_start in range(0, len(images), batch_size):
                    batch = images[batch_start:batch_start + batch_size]
                    results = pool.imap(scorecards_parser.parse_image, batch)

                    # Process results as they complete
                    batch_results = [r.to_list() for r in results]
